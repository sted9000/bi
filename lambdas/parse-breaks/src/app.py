...
def lambda_handler(event, context):
    ...
    # set the file key
    file_key = f"employee-breaks-{event_date}.pdf"
    ...
    try:
        ...
        for store in stores:
            ...
            # Extract the number (can include a decimal point) before the word 'Page'
            # If there is no number (like in the first example above), the default value is 0
            # And also match if the number is a cent (like in the last example above)
            match = re.search(r'(\d*\.\d+|\d+)\s*Page', text)
            if match:
            ...
            supabase.table('break').insert(
                {'store_id': store['id'], 'total': total, 'date': event_date}
            ).execute()
